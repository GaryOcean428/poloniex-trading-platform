...
class ComprehensiveTestSuite {
  ...
  async runCoreServiceTests() {
    ...
    await this.runTest('Paper Trading Service Initialization', async () => {
      ...
      const positionSize = paperTradingService.calculatePositionSize(
        session,
        { strength: 0.8, side: 'long' },
        { price: 50000 }
      );
  ...
  async runIntegrationTests() {
    ...
    await this.runTest('Paper Trading-Risk Management Integration', async () => {
      ...
      const riskCheck = paperTradingService.performRiskCheck(
        mockSession,
        mockSignal,
        mockMarketData
      );
  ...
  async runRiskManagementTests() {
    ...
    await this.runTest('Stop Loss & Take Profit Logic', async () => {
      ...
      for (const price of testPrices) {
        const exitSignal = backtestingEngine.checkExitConditions(
          mockPosition,
          { close: price },
          mockPosition.stopLoss,
          mockPosition.takeProfit
        );
    ...
    await this.runTest('Portfolio Risk Limits', async () => {
      ...
      const riskCheck1 = paperTradingService.performRiskCheck(
        { ...mockSession, currentValue: 95000 }, // Exactly at limit
        mockSignal,
        mockMarketData
      );

      // Test 2: Below risk limit (should be allowed)
      const riskCheck2 = paperTradingService.performRiskCheck(
        { ...mockSession, currentValue: 97000 }, // Below limit
        mockSignal,
        mockMarketData
      );
  ...
  async runEndToEndTests() {
    ...
    await this.runTest('Complete Trading Cycle', async () => {
      ...
      const closedPosition = await paperTradingService.closePosition(
        session.id,
        position.id,
        'test_close',
        51000
    ...
    await this.runTest('Automated Decision Making', async () => {
      ...
      const confidenceScore = 75;
      const recommendedSize = confidenceScoringService.calculateRecommendedPositionSize(
        confidenceScore,
        marketConditions
      );
