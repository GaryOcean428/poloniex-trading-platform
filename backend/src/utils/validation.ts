import { Strategy, StrategyParameters, StrategyPerformance } from '@shared/types/strategy';

/**
 * Type guard to check if an object has the basic structure of a Strategy
 */
export function isStrategy(obj: unknown): obj is Strategy {
 ...
 */
export function isStrategyParameters(obj: unknown): obj is StrategyParameters {
  if (!obj || typeof obj !== 'object') {
    return false;
  }
...
export function isStrategyPerformance(obj: unknown): obj is StrategyPerformance {
  ...
  const performance = obj as Record<string, unknown>;
  
  return (
    typeof performance.totalPnL === 'number' &&
 ...
 */
export function validateStrategies(strategies: unknown[]): strategies is Strategy[] {
 ...
 */
export function sanitizeStrategy(obj: unknown): Strategy | null {
 ...
 */
export function assertStrategy(obj: unknown, context = 'Unknown'): Strategy {
  if (!validateStrategy(obj)) {
 ...
 */
export function assertStrategies(obj: unknown, context = 'Unknown'): Strategy[] {
 ...
 */
export function sanitizeStrategyUpdate(updates: Record<string, unknown>): Partial<Strategy> {
  const sanitized: Partial<Strategy> = {};
  ...
  if (typeof updates.type === 'string' && ['manual', 'automated', 'ml', 'dqn'].includes(updates.type)) {
    sanitized.type = updates.type as Strategy['type'];
  }
  if (typeof updates.active === 'boolean') sanitized.active = updates.active;
  if (typeof updates.algorithm === 'string') sanitized.algorithm = updates.algorithm as Strategy['algorithm'];
