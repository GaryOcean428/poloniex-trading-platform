...
interface AgentConfig {
  ...
  strategyGenerationInterval: number; // Hours between strategy generation
...
interface AgentLearning {
  sessionId: string;
  learningType: 'strategy_success' | 'strategy_failure' | 'market_pattern' | 'risk_event';
  context: any;
  insight: string;
  confidence: number;
}
...
class AutonomousTradingAgent extends EventEmitter {
  ...
  async startAgent(userId: string, config: Partial<AgentConfig>): Promise<AgentSession> {
    ...
    const result = await pool.query(
      `INSERT INTO agent_sessions (user_id, status, started_at, config)
       VALUES ($1, $2, NOW(), $3)
       RETURNING *`,
      [userId, 'running', JSON.stringify(defaultConfig)]
    );
  ...
  async stopAgent(sessionId: string): Promise<void> {
    ...
    await pool.query(
      `UPDATE agent_sessions SET status = $1, stopped_at = NOW() WHERE id = $2`,
      ['stopped', sessionId]
    );
   ...
   */
  private async generateStrategies(session: AgentSession): Promise<AgentStrategy[]> {
    ...
    for (let i = 0; i < numStrategies; i++) {
      try {
        ...
        // Save to database
        await pool.query(
          `INSERT INTO agent_strategies (id, session_id, strategy_name, strategy_code, generation_prompt, claude_response, status)
           VALUES ($1, $2, $3, $4, $5, $6, $7)`,
          [strategy.id, strategy.sessionId, strategy.strategyName, strategy.strategyCode, 
    ...
    await pool.query(
      `UPDATE agent_sessions SET strategies_generated = strategies_generated + $1 WHERE id = $2`,
      [strategies.length, session.id]
    );
   ...
   */
  private async backtestStrategies(
    ...
    const backtestPromises = strategies.map(async (strategy) => {
      try {
        ...
        // Update database
        await pool.query(
          `UPDATE agent_strategies SET backtest_score = $1, status = $2 WHERE id = $3`,
          [score, 'backtested', strategy.id]
        );

        await this.logActivity(
          session.id,
          'backtest_completed',
          `Backtest completed for ${strategy.strategyName}: Score ${score.toFixed(2)}`
        );
    ...
    const successfulResults = results.filter(r => r !== null) as AgentStrategy[];

    // Update session
    session.backtestsCompleted += successfulResults.length;
    await pool.query(
      `UPDATE agent_sessions SET backtests_completed = backtests_completed + $1 WHERE id = $2`,
      [successfulResults.length, session.id]
    );
   ...
   */
  private selectTopStrategies(strategies: AgentStrategy[], topN: number): AgentStrategy[] {
    return strategies
   ...
   */
  private async startPaperTrading(session: AgentSession, strategies: AgentStrategy[]): Promise<void> {
    for (const strategy of strategies) {
      try {
        ...
        await pool.query(
          `UPDATE agent_strategies SET status = $1 WHERE id = $2`,
          ['paper_trading', strategy.id]
        );

        await this.logActivity(
          session.id,
          'paper_trading_started',
          `Started paper trading for ${strategy.strategyName}`
        );
   ...
   */
  private async evaluatePaperTradingResults(session: AgentSession): Promise<void> {
    // Get strategies currently in paper trading
    const result = await pool.query(
      `SELECT * FROM agent_strategies 
       WHERE session_id = $1 AND status = 'paper_trading'`,
      [session.id]
    );
   ...
   */
  private async learnAndAdapt(session: AgentSession): Promise<void> {
    ...
    for (const learning of learnings) {
      await pool.query(
        `INSERT INTO agent_learnings (session_id, learning_type, context, insight, confidence)
         VALUES ($1, $2, $3, $4, $5)`,
        [learning.sessionId, learning.learningType, JSON.stringify(learning.context), 
    ...
    await this.logActivity(
      session.id,
      'learning_completed',
      `Analyzed ${learnings.length} strategy outcomes`
    );
  }
   ...
   */
  private buildStrategyGenerationPrompt(config: AgentConfig, marketContext: any, index: number): string {
   ...
   * Helper: Log activity
   */
  private async logActivity(sessionId: string, activityType: string, description: string): Promise<void> {
    await pool.query(
      `INSERT INTO agent_activity_log (session_id, activity_type, description)
       VALUES ($1, $2, $3)`,
