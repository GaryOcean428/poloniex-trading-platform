...
class AutomatedTradingService extends EventEmitter {
  ...
  async updateStrategyAllocation(userId, strategyId, newPositionSize) {
    try {
      ...
      try {
        await query(
          `UPDATE strategies SET position_size = $1, updated_at = NOW() WHERE id = $2 AND user_id = $3`,
          [strategy.positionSize, strategyId, userId]
        );
      } catch (dbError) {
      ...
      logger.info(
        `Updated allocation for strategy ${strategyId}: ${(strategy.positionSize * 100).toFixed(2)}%`
      );
