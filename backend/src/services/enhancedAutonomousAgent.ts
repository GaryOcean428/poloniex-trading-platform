  ...
  async generateStrategies(session: AgentSession): Promise<Strategy[]> {
    ...
    for (const symbol of session.config.preferredPairs) {
      try {
        ...
        const trendStrategy = await this.generateSingleStrategy(
          session,
          symbol,
          'trend_following',
          ['SMA', 'EMA'],
        ...
        const momentumStrategy = await this.generateSingleStrategy(
          session,
          symbol,
          'momentum',
          ['RSI', 'MACD'],
        ...
        const volumeStrategy = await this.generateSingleStrategy(
          session,
          symbol,
          'volume_analysis',
          ['Volume', 'OBV'],
