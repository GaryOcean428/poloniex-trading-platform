...
import qigMetrics, { MarketState, QIGMetrics } from './qigMetrics.js';
...
class QIGEnhancedMLService {
   ...
   */
  private buildMarketState(ohlcvData: OHLCVData[]): MarketState {
   ...
   */
  private geometricRegimePrediction(
    state: MarketState,
    metrics: QIGMetrics,
    currentPrice: number
  ): { '1h': Prediction; '4h': Prediction; '24h': Prediction } {
   ...
   */
  private breakdownRegimePrediction(
    state: MarketState,
    metrics: QIGMetrics,
    currentPrice: number
  ): { '1h': Prediction; '4h': Prediction; '24h': Prediction } {
  ...
  private generateExplanation(metrics: QIGMetrics, state: MarketState): string {
    ...
    const topIndicators = sortedWeights.map(([name, weight]) => 
