...
router.get('/orders', authenticateToken, async (req, res) => {
  try {
    ...
    const { symbol } = req.query;
    const orders = await poloniexFuturesService.getCurrentOrders(credentials, symbol);
    res.json(orders);
  } catch (error) {
...
router.delete('/orders', authenticateToken, async (req, res) => {
  try {
    ...
    const { orderId, clientOid } = req.body;
    if (!orderId && !clientOid) {
      return res.status(400).json({
        error: 'Either orderId or clientOid is required'
      });
    }

    const result = await poloniexFuturesService.cancelOrder(credentials, orderId, clientOid);
    res.json(result);
  } catch (error) {
