...
router.get('/settings', authenticateToken, async (req, res) => {
    try {
        ...
        try {
            const result = await pool.query('SELECT * FROM risk_settings WHERE user_id = $1', [userId]);
...
router.put('/settings', authenticateToken, async (req, res) => {
    try {
        const userId = String(req.user.id);
        const { maxDrawdown, maxPositionSize, maxConcurrentPositions, stopLoss, takeProfit, dailyLossLimit, maxLeverage, riskLevel } = req.body;
        // Validate inputs
        ...
        try {
            await pool.query(`INSERT INTO risk_settings (
