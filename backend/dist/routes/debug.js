...
router.get('/users', async (req, res) => {
    ...
    `);
        ...
        const demoUserResult = await pool.query('SELECT id, username, email, created_at FROM users WHERE LOWER(username) = $1 OR LOWER(email) = $1', ['demo']);
        // Check if GaryOcean user exists
        const garyUserResult = await pool.query('SELECT id, username, email, created_at FROM users WHERE LOWER(username) = $1', ['garyocean']);
        // Get all usernames (without sensitive data)
        const allUsersResult = await pool.query('SELECT id, username, email, created_at FROM users ORDER BY created_at DESC LIMIT 10');
        res.json({
            success: true,
