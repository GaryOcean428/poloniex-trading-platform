...
export class ImprovedApiCredentialsService {
     ...
     */
    async storeCredentials(userId, apiKey, apiSecret, exchange = 'poloniex', credentialName) {
        try {
            // Validate inputs
            ...
            const result = await pool.query(`INSERT INTO api_credentials (
        ...
        RETURNING id`, [userId, exchange, encrypted.apiKeyEncrypted, encrypted.apiSecretEncrypted, encrypted.encryptionIv, encrypted.tag]);
    ...
    async getCredentials(userId, exchange = 'poloniex') {
        try {
         ...
         LIMIT 1`, [userId, exchange]);
            ...
            // Decrypt credentials
            try {
                const decrypted = encryptionService.decryptCredentials(stored.api_key_encrypted, stored.api_secret_encrypted, stored.encryption_iv, stored.encryption_tag);
     ...
     */
    async validateCredentials(userId, exchange = 'poloniex') {
        try {
     ...
     */
    async getCredentialsStatus(userId, exchange = 'poloniex') {
        try {
            const result = await pool.query(`SELECT is_active, encryption_tag, last_used_at, created_at
         FROM api_credentials
