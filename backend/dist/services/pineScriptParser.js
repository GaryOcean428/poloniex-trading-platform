...
export class PineScriptParser {
    ...
    static parse(script) {
        ...
        if (strategyLine) {
            ...
            if (overlayMatch)
                strategy.overlay = overlayMatch[1] === 'true';
        }
        // Parse indicators
        ...
        for (const line of lines) {
            if (line.includes('strategy.entry(') && line.includes('long')) {
                ...
                if (condition)
                    strategy.entryConditions.long.push(condition);
            }
            if (line.includes('strategy.entry(') && line.includes('short')) {
                const condition = this.parseEntryCondition(line, lines);
                if (condition)
                    strategy.entryConditions.short.push(condition);
            }
        }
            ...
            if (line.includes('strategy.exit(') || line.includes('strategy.close(')) {
                ...
                if (exitInfo) {
                    if (exitInfo.stopLoss)
                        strategy.riskManagement.stopLoss = exitInfo.stopLoss;
                    if (exitInfo.takeProfit)
                        strategy.riskManagement.takeProfit = exitInfo.takeProfit;
                    if (exitInfo.trailingStop)
                        strategy.riskManagement.trailingStop = exitInfo.trailingStop;
                }
            }
        }
    ...
    static parseEntryCondition(line, allLines) {
...
`;
    ...
    static generateIndicatorCode(indicator) {
