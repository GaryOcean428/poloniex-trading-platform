...
class AgentSettingsService {
    ...
    async saveSettings(userId, settings) {
        try {
            const result = await pool.query(`INSERT INTO agent_settings (user_id, run_mode, auto_start_on_login, continue_when_logged_out, config)
         VALUES ($1, $2, $3, $4, $5)
     ...
     */
    async getAlwaysRunUsers() {
        try {
            const result = await pool.query(`SELECT user_id FROM agent_settings WHERE run_mode = 'always'`);
     ...
     */
    async getAutoStartUsers() {
        try {
            const result = await pool.query(`SELECT user_id FROM agent_settings WHERE auto_start_on_login = true`);
            return result.rows.map(row => row.user_id);
        }
        catch (error) {
