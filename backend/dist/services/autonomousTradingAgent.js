        ...
        for (const row of result.rows) {
            ...
            if (hoursPassed >= session.config.paperTradingDurationHours) {
             ...
             WHERE id = $3`, ['live', paperTradingScore, row.id]);
                    await this.logActivity(session.id, 'strategy_promoted', `Strategy ${row.strategy_name} promoted to live trading`);
             ...
             WHERE id = $3`, ['retired', paperTradingScore, row.id]);
                    await this.logActivity(session.id, 'strategy_retired', `Strategy ${row.strategy_name} retired (poor paper trading performance)`);
                }
            }
        }
    }
