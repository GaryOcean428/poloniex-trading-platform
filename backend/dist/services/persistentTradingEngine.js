...
export class PersistentTradingEngine extends EventEmitter {
    constructor() {
        ...
        this.executionIntervalMs = 5000; // Check every 5 seconds
    ...
    async loadActiveSessions() {
        try {
            const result = await pool.query(`SELECT * FROM trading_sessions WHERE is_active = true`);
     ...
     */
    async initializeSessionContext(session) {
        try {
     ...
     */
    async executeSessionStrategy(context) {
        ...
        try {
            ...
            const positions = await poloniexService.getPositions({ apiKey: context.credentials.apiKey, apiSecret: context.credentials.apiSecret }, strategyConfig.symbol);
            ...
            const signal = await this.generateTradingSignal(strategyConfig, marketData, positions, positionState);
     ...
     */
    async generateTradingSignal(strategyConfig, marketData, positions, positionState) {
        try {
            ...
            const signal = await automatedTradingService.executeStrategyLogic(strategy, marketData, positions);
     ...
     */
    async executeTrade(context, signal, marketData) {
    ...
    async updateSessionHeartbeat(sessionId) {
        try {
            await pool.query(`UPDATE trading_sessions 
    ...
    async updateSessionMetrics(sessionId, metrics) {
        try {
         ...
         WHERE id = $2`, [JSON.stringify(metrics), sessionId]);
        }
        catch (error) {
     ...
     */
    async startSession(userId, strategyConfig, riskConfig, sessionName) {
        try {
            // Create new session in database
            const result = await pool.query(`INSERT INTO trading_sessions (
        ...
        RETURNING id`, [
            ...
            const sessionResult = await pool.query(`SELECT * FROM trading_sessions WHERE id = $1`, [sessionId]);
