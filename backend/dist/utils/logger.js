...
const logFormat = winston.format.combine(winston.format.timestamp(), winston.format.errors({ stack: true }), winston.format.printf(({ timestamp, level, message, stack, ...metadata }) => {
    ...
    if (metadataKeys.length > 0 && metadataKeys.some(key => !EXCLUDED_METADATA_KEYS.includes(key))) {
        const cleanMetadata = Object.fromEntries(Object.entries(metadata).filter(([key]) => !EXCLUDED_METADATA_KEYS.includes(key)));
