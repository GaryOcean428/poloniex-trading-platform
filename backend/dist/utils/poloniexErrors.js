...
export function parsePoloniexError(error) {
    ...
    if (error.code === 'ECONNREFUSED' || error.code === 'ENOTFOUND') {
        return new PoloniexAPIError('Network connection failed', 'NETWORK_ERROR', 0, { originalError: error.message });
    }
    // Timeout errors
    if (error.code === 'ECONNABORTED' || error.code === 'ETIMEDOUT') {
        const timeoutError = new PoloniexAPIError('Request timeout', 'TIMEOUT', 0, { originalError: error.message });
        timeoutError.isRetryable = true;
        return timeoutError;
    }
    // No response from server
    if (!error.response) {
        return new PoloniexAPIError('No response from server', 'NO_RESPONSE', 0, { originalError: error.message });
    }
    ...
    if (errorInfo) {
        ...
        const parsedError = new ErrorClass(errorInfo.message, errorCode, {
    ...
    // Unknown error
    return new PoloniexAPIError(errorMessage, errorCode, status, { originalResponse: data });
}
/**
