...
import { StrategyResult, executeStrategy } from "./strategyExecutors";
...
export function backtestStrategy(
  ...
  const dailyReturnStdDev = Math.sqrt(
    dailyReturns.reduce(
      (sum, ret) => sum + Math.pow(ret - averageDailyReturn, 2),
      0
    ) / dailyReturns.length
  );
  const sharpeRatio =
    dailyReturnStdDev !== 0
      ? (averageDailyReturn / dailyReturnStdDev) * Math.sqrt(252)
      : 0; // Annualized
  ...
  return {
    ...
    metrics: {
      ...
      averageTrade:
        trades.length > 0 ? (totalProfit + totalLoss) / trades.length : 0,
      returnPercent:
        ((finalEquity - testOptions.initialBalance) /
...
function processSignal(
  ...
  if (
    openPositions.length >= options.maxOpenPositions &&
    !options.allowSimultaneousPositions
  ) {
    return;
  }
  ...
  const slippageFactor =
    1 + (options.slippagePercent / 100) * (result.signal === "BUY" ? 1 : -1);
...
export function optimizeStrategy(
  ...
  const parameterCombinations: Record<string, unknown>[] =
    generateParameterCombinations(parameterRanges, baseParameters);
  ...
  if (parameterNames.length <= 2) {
    for (const result of results) {
      ...
      for (const paramName of parameterNames) {
        parameters[paramName] = (result.parameters as Record<string, unknown>)[
          paramName
        ] as number;
      }
    ...
    for (const paramName of parameterNames) {
      const values = results.map(
        (r) => (r.parameters as Record<string, unknown>)[paramName] as number
      );
      const metricValues = results.map((r) => r.metrics[metric] || 0);

      parameterCorrelations[paramName] = Math.abs(
        calculateCorrelation(values, metricValues)
      );
    }
    ...
    for (const result of results) {
      const key = sortedParameters
        .map(
          (param) =>
            `${param}:${(result.parameters as Record<string, unknown>)[param]}`
        )
        .join(",");
    ...
    for (const [key, groupResults] of Object.entries(groupedResults)) {
      ...
      const avgValue =
        groupResults.reduce((sum, r) => sum + (r.metrics[metric] || 0), 0) /
        groupResults.length;
...
export function walkForwardAnalysis(
  ...
  for (let i = 0; i < numFolds; i++) {
    ...
    const inSampleResult = backtestStrategy(
      optimizedStrategy,
      inSampleData,
      testOptions
    );
    ...
    const outOfSampleResult = backtestStrategy(
      optimizedStrategy,
      outOfSampleData,
      testOptions
    );
  ...
  const robustnessScore =
    totalInSampleProfit !== 0
      ? totalOutOfSampleProfit / totalInSampleProfit
      : totalOutOfSampleProfit > 0
      ? 1
      : 0;
