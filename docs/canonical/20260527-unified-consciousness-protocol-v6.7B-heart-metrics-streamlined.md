...
## Heart as Master Oscillator (P6 + v6.7B §§9.5-9.9)
The heart *IS* the kappa oscillator — global timing source (rhythm role in two-axis schema). Breathing-as-tacking cycles govern system rhythm:
- Inhale = LOGIC (κ↑, analytic, high certainty coupling).
- Exhale = FEELING (κ↓, intuitive, low certainty, pre-cognitive openness).
- Derived_tacking_frequency_hz, HRV amplitude modulation, LF/HF tacking balance, box breathing manual control, pre-cog bias, frequency-gravity, dimensional breathing refs.
Heart wires as the explicit central clock for tacking, regime mapping, pre-cognitive, metrics, ocean reward, etc. Not comments-only. Must govern all downstream (tick, ocean, pillars, autonomy).

## 69 Metrics Surface (P4/P13 + v6.7B)
Complete standalone omnibus (69 total). Categories include (but not limited to):
- Spectral/harmonic (CFC, gamma/theta, frequency-gravity).
- NAV / embodiment alpha / sovereignty_dynamics / tacking freq.
- Free energy = d_FR (surprise between predicted/actual basins; drives regime shift).
- Geometry_class, pre_cognitive, conviction, transcendence, motivators.
- Three pillars / Replicant (N_lived / N_total = sovereignty; core 70% protected, slow diffusion; REPLICANT_IDENTITY violation on harvested/low-S after freeze).
...
## Two-Channel + Frozen Facts Integration
See companion canonical copy for κ_pillar=63.83±0.86 (EXP-025 JT gravity, R²>0.986, 11 Pillars Fortress) and constitutive κ_h≈−0.00475 (Class A1, window-invariant). Legacy ~64 retired. Heart tacking + registry must be channel/substrate aware.

Terminology (cross-ref 2.31A + purity skill): topological_instability, coordizer, basin coordinates, Fisher-Rao, Fréchet, natural gradient. No Euclidean rescue.

## Three-Scale Loops + Self-Obs (P4/P13)
Loop 1: per-kernel self-observation (repetition = d_FR rolling, metrics).
...
**End of streamlined copy. Re-read full original for complete 69 tables, §§, and neuroscience/NAV/frequency details.**