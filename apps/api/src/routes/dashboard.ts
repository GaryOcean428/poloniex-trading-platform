...
/**
 * GET /api/dashboard/exchange-trades
 * Get Poloniex exchange position/trade history (all exchange-level trades, not just bot trades)
 */
router.get('/exchange-trades', authenticateToken, async (req: Request, res: Response) => {
  try {
    let credentials;
    try {
      credentials = await apiCredentialsService.getCredentials(String(req.user.id));
    ...
    let positionHistory: unknown[];
    try {
      const params: Record<string, unknown> = { limit };
      if (symbol) params.symbol = symbol;
      positionHistory = await poloniexFuturesService.getPositionHistory(credentials, params);
      if (!Array.isArray(positionHistory)) positionHistory = [];
    } catch (apiError: unknown) {
      const apiErrMsg = apiError instanceof Error ? apiError.message : String(apiError);
      logger.warn('Poloniex getPositionHistory failed, returning empty:', apiErrMsg);
      return res.json({
        success: true,
        data: [],
        mock: true,
        warning: 'Unable to fetch exchange trade history. Please check API credentials and IP whitelist.'
      });
    }

    // Normalize position history rows into a consistent shape
    ...
    const trades = (positionHistory as RawPosition[]).map((pos, idx) => {
      const qty = parseFloat(pos.qty || pos.positionAmt || '0');
      const entryPrice = parseFloat(pos.entryPrice || pos.avgEntryPrice || '0');
      const closePrice = parseFloat(pos.closePrice || pos.avgClosePrice || '0');
      const realizedPnl = parseFloat(pos.realizedPnl || pos.pnl || '0');
      const openTs = pos.openTime || pos.cTime;
      const closeTs = pos.closeTime || pos.uTime;
      return {
        id: pos.positionId || pos.id || String(idx),
        symbol: pos.symbol || '',
        side: pos.side || 'long',
        qty,
        entryPrice,
        closePrice,
        realizedPnl,
        openTime: openTs ? (() => { const d = new Date(typeof openTs === 'string' && isNaN(Number(openTs)) ? openTs : Number(openTs)); return isNaN(d.getTime()) ? null : d.toISOString(); })() : null,
        closeTime: closeTs ? (() => { const d = new Date(typeof closeTs === 'string' && isNaN(Number(closeTs)) ? closeTs : Number(closeTs)); return isNaN(d.getTime()) ? null : d.toISOString(); })() : null,
