...
import { MarketState } from './qigMetrics.js';
...
export class MarketStatePredictor {
  ...
  private readonly predictionWindow = 5; // Use last 5 states for prediction
   ...
   */
  private computeExponentialWeights(n: number): number[] {
   ...
   */
  private predictPrices(recentStates: StateHistory[]): number[] {
   ...
   */
  private predictVolume(recentStates: StateHistory[]): number[] {
    if (recentStates.length === 0) return [];
   ...
   */
  private cloneState(state: MarketState): MarketState {
    return {
      prices: [...state.prices],
