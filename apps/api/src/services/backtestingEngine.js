...
class BacktestingEngine extends EventEmitter {
   ...
   * Guards against non-array responses (rate limits, error objects, null).
   */
  async fetchHistoricalDataFromAPI(symbol, timeframe, startDate, endDate) {
    try {
      ...
      // Poloniex API may return error objects, null, or non-array responses
      // (e.g. rate limiting, invalid symbol, server errors)
      if (!Array.isArray(klineData)) {
        logger.warn(`Poloniex klines API returned non-array for ${symbol} (${timeframe}): ${typeof klineData}${klineData && typeof klineData === 'object' ? ' — ' + JSON.stringify(klineData).slice(0, 200) : ''}`);
        return [];
      }

      if (klineData.length === 0) {
        logger.warn(`Poloniex klines API returned empty array for ${symbol} (${timeframe})`);
        return [];
      }
  ...
  generateMomentumSignals(indicators, params) {
    ...
    if (indicators.rsi < rsi_oversold && indicators.macd.histogram > macd_threshold) {
      return { side: 'long', strength: Math.abs(indicators.rsi - 50) / 50, reason: 'momentum_long' };
    }
    if (indicators.rsi > rsi_overbought && indicators.macd.histogram < -macd_threshold) {
      return { side: 'short', strength: Math.abs(indicators.rsi - 50) / 50, reason: 'momentum_short' };
    }
    return null;
  }
  ...
  generateMeanReversionSignals(indicators, params) {
    ...
    if (currentPrice < lower && indicators.rsi < rsi_extreme) {
      return { side: 'long', strength: (lower - currentPrice) / (upper - lower), reason: 'mean_reversion_long' };
    }
    if (currentPrice > upper && indicators.rsi > (100 - rsi_extreme)) {
      return { side: 'short', strength: (currentPrice - upper) / (upper - lower), reason: 'mean_reversion_short' };
    }
    return null;
  }
  ...
  generateTrendFollowingSignals(indicators, params) {
    ...
    if (shortMA > longMA) {
      ...
      return { side: 'long', strength: Math.min(spread * 20, 1), reason: 'trend_following_long' };
    }
    if (shortMA < longMA) {
      const spread = (longMA - shortMA) / longMA;
      return { side: 'short', strength: Math.min(spread * 20, 1), reason: 'trend_following_short' };
    }
    return null;
  }
  ...
  generateBreakoutSignals(indicators, params) {
    ...
    if (currentPrice > upper && volumeConfirmed) {
      return { side: 'long', strength: Math.min((currentPrice - upper) / (upper - lower), 1), reason: 'breakout_long' };
    }
    if (currentPrice < lower && volumeConfirmed) {
      return { side: 'short', strength: Math.min((lower - currentPrice) / (upper - lower), 1), reason: 'breakout_short' };
    }
    return null;
  }
  ...
  async executeEntry(signal, currentCandle, config) {
    try {
      ...
      const position = {
        id: `pos_${Date.now()}`, symbol: config.symbol, side: signal.side, size: positionSize,
        entryPrice: executionPrice, entryTime: currentCandle.timestamp, stopLoss, takeProfit,
        unrealizedPnl: 0, status: 'open'
      };
      const trade = {
        id: `trade_${Date.now()}`, positionId: position.id, symbol: config.symbol, side: signal.side,
        size: positionSize, price: executionPrice, timestamp: currentCandle.timestamp, type: 'entry',
        reason: signal.reason, fees: this.calculateTradingFees(positionSize, executionPrice)
      };
  ...
  calculateBollingerBands(values, period = 20, stdDev = 2) {
    ...
    return { middle: sma, upper: sma + (standardDeviation * stdDev), lower: sma - (standardDeviation * stdDev) };
  }
  ...
  checkExitConditions(position, currentCandle, stopLoss, takeProfit) {
    ...
    if (position.side === 'long' && currentPrice <= stopLoss) return { reason: 'stop_loss', price: stopLoss };
    if (position.side === 'short' && currentPrice >= stopLoss) return { reason: 'stop_loss', price: stopLoss };
    if (position.side === 'long' && currentPrice >= takeProfit) return { reason: 'take_profit', price: takeProfit };
    if (position.side === 'short' && currentPrice <= takeProfit) return { reason: 'take_profit', price: takeProfit };
    return null;
  }
  ...
  async executeExit(position, currentCandle, reason) {
    try {
      const exitPrice = this.simulateMarketExecution(
        currentCandle.close, position.side === 'long' ? 'short' : 'long', position.size, 'exit'
      );
      const pnl = this.calculatePnL(position, exitPrice);
      const trade = {
        id: `trade_${Date.now()}`, positionId: position.id, symbol: position.symbol,
        side: position.side === 'long' ? 'short' : 'long', size: position.size, price: exitPrice,
        timestamp: currentCandle.timestamp, type: 'exit', reason,
        fees: this.calculateTradingFees(position.size, exitPrice), pnl
      };
      ...
      if (pnl > 0) this.currentBacktest.metrics.winningTrades++;
      else this.currentBacktest.metrics.losingTrades++;
      this.currentBacktest.trades.push(trade);
      return trade;
    } catch (error) {
  ...
  updatePortfolioAfterTrade(trade, type) {
    ...
    this.currentBacktest.portfolio.totalValue = 
      this.currentBacktest.portfolio.cash + this.currentBacktest.portfolio.margin +
  ...
  updatePortfolioValue(currentCandle, position) {
    ...
    this.currentBacktest.portfolio.totalValue = 
      this.currentBacktest.portfolio.cash + this.currentBacktest.portfolio.margin + unrealizedPnl;
  }
  ...
  calculateBacktestMetrics() {
    ...
    for (const point of equity_curve) {
      ...
      if (drawdown > maxDrawdown) { maxDrawdown = drawdown; maxDrawdownPercent = drawdownPercent; }
    }
    ...
    this.currentBacktest.metrics = {
      ...
      sharpeRatio, sortinoRatio, calmarRatio: maxDrawdownPercent > 0 ? totalReturn / maxDrawdownPercent : 0,
  ...
  calculateExpectancy(trades) {
    ...
    return exitTrades.reduce((sum, t) => sum + t.pnl, 0) / exitTrades.length;
  }

  calculateStopLoss(entryPrice, side, config) {
    const stopLossPercent = config.stopLossPercent || 0.02;
    return side === 'long' ? entryPrice * (1 - stopLossPercent) : entryPrice * (1 + stopLossPercent);
  }

  calculateTakeProfit(entryPrice, side, config) {
    const takeProfitPercent = config.takeProfitPercent || 0.04;
    return side === 'long' ? entryPrice * (1 + takeProfitPercent) : entryPrice * (1 - takeProfitPercent);
  }
  ...
  async getBacktestResults(limit = 10) {
    try {
      const results = await query('SELECT * FROM backtest_results ORDER BY created_at DESC LIMIT $1', [limit]);
      return results.rows;
    } catch (error) {
  ...
  async getBacktestDetails(backtestId) {
    try {
      ...
      if (backtestResult.rows.length === 0) return null;
      return { backtest: backtestResult.rows[0], trades: tradesResult.rows, equityCurve: equityCurveResult.rows };
    } catch (error) {
